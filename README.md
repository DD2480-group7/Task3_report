...
The user can be part of multiple realms (organizations) with a single account as one server can host multiple realms. This can be compared to Slack which has a similar (but premium) feature, meaning that the default is a different accounts for each Slack team, which would correspeond to for each realm in Zulip. Each realm is private and only visable to users who have joined the realm. Realms have assigned Administrators who can allow new users to register and join. Administrators can issue invitations to members which enable them to join the realm and they can also prevent specific members from joining.

Zulip offers a summary of all unread messages from subscribed streams in a tab called "All messages". The tab also includes messages sent from other users as private messages. The tab displays all the messages in chronological order. There are also separate inboxes for private messages and mentions. All streams which the user has subscribed to are displayed in a list and have indiviudal settings which the user can modify, such as muting the stream or unsubscriing from it entirely. 


The user can manipulate the view to create a narrow, focused view. E.g. the view can be narrowed down to focus on one topic within a stream, this is called a narrow. An additional functionality of these narrows is that each have their own unique URL which enables users to link to a specific topic directly. Each conversation view has breadcrumbs with the stream name and topic displayed on the bar in the top of the interface which makes it easy to navigate different conversations in a stream.
